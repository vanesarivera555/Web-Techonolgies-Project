...
//we take err message and put it in url, if we do then we can check to see if it is set on the url. If it set on the url mean we have error, so then we have to set them appropriatly. -->
<?php	
session_start();
...
//check url, if msg is set and msg is successful, take the value we are expecting.
if (isset($_GET['msg']) && $_GET['msg']=="success")
echo '<div class="alert alert-success" role="alert">Your entry has been recevied!</div>';
...
//will either execute properly or spit out an error. For produciton, error gets put on database. 
// will either get a successful execution or die and redirect
...
echo '<div class="text-center">Assignment 16 PHP</div></div>';
echo '</div>';
echo '</body>';
echo '</html>';
echo '';           
